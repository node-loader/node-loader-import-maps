...
function resolveImportsMatch(normalizedSpecifier, specifierMap) {
  for (let specifierKey in specifierMap) {
    ...
    if (specifierKey === normalizedSpecifier) {
      ...
      const afterPrefix = normalizedSpecifier.slice(specifierKey.length + 1);
      try {
...
function sortAndNormalizeSpecifierMap(map, baseURL) {
  ...
  for (let specifierKey in map) {
    ...
    if (addressURL === null) {
      console.warn(
        `Invalid URL address for import map specifier '${specifierKey}' is invalid`
      );
      normalized[specifierKey] = null;
      continue;
    }
    ...
    normalized[specifierKey] = addressURL;
  }
...
function parseURLLikeSpecifier(specifier, baseURL) {
  ...
  try {
    return new URL(specifier, useBaseUrlAsParent ? baseURL : null).href;
  } catch {
    return null;
  }
}
